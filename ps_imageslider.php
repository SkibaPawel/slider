...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    {		
		  ...
		  // sprawdzamy jaki czy mobile 	    
		//echo('getWidgetVariables'); die; 	
		
	if(Context::getContext()->isMobile()  || Context::getContext()->isTablet()){
		$slides = $this->getSlides(true , true);
	}else{
    ...
    public function getSlides($active = null , $mobile = false)
    {
		
		// var_dump( $this->context->controller) ; die;
		...
		$condition =  ''; 
		if($active){
			$condition .= ' AND hss.`active` = 1 ' ; 
			if( $mobile){
				$condition .= ' AND hss.`mobile` = 1 ' ; 				
			}else{
				$condition .= ' AND hss.`mobile` = 0 ' ; 
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            AND hssl.id_lang = '.(int)$id_lang.$condition.' 
    ...
    public function displayMobile($id_slide, $active)
    {
        $title = ((int)$active == 0 ? $this->getTranslator()->trans('Mobile', array(), 'Admin.Global') : $this->getTranslator()->trans('Mobile', array(), 'Admin.Global'));
        $icon = ((int)$active == 0 ? 'icon-remove' : 'icon-check');
        $class = ((int)$active == 0 ? 'btn-danger' : 'btn-success');
        $html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
            '&configure='.$this->name.
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Mobile', array(), 'Admin.Global'),
                        'name' => 'mobile_slide',
                        'is_bool' => true,
                        'values' => array(
                            array(
